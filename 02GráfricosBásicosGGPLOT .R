...
devtools::install_github("ricardo-bion/ggradar") ##Grafico Radar no oficial de R descargado e instalado de github
...
ENIGH %>% 
  ...
  ungroup() %>% 
  as.data.frame() %>% 
  write.csv(file = "Bases/ENIGH/Ingreso_promedio_ENT.csv",
            fileEncoding = "UTF-8",
            row.names = F)

Base <- read.csv("Bases/ENIGH/Ingreso_promedio_ENT.csv",
                 encoding = "UTF-8",
                 check.names = F)
...
data %>% 
  ...
  labs(title = "Niveles de consumo de *combustible* por cantidad de *caballos de fuerza*",
       subtitle = "para 32 vehiculos de los años 1972-1973",
       caption = "Elaboración propia con la base mtcars",
       x="Caballos de Fuerza (HP)",
       y="Millas por galon(MPG)")+
  
  theme(#legend.position = "bottom",
        panel.background = element_rect(fill="white"),#fondo de la geometria
        plot.background = element_rect(fill = alpha("#45b39d",0.5)), #fondo fuera de geometria
        legend.background = element_rect(fill = alpha("#45b39d",0.5)),#fondo de la leyenda
        legend.key = element_rect(fill= alpha("#45b39d",0.5)),#Fonfo del elemento de la leyenda 
        panel.grid.major = element_line(colour = alpha("#7f8c8d", alpha=0.5)),#color de lineas
        panel.grid.minor = element_blank(),
        plot.title = ggtext::element_markdown(hjust=0.5,#Centra  titulo 
                                              size=18,#Tamaño de letra
                                              colour="black"),#formato de letra, negrita  y en cursiva ademas pone el color de letra
        plot.subtitle = ggtext::element_markdown(hjust=0.5,#Formato de subtitulo
                                                 size=16,
                                                 colour="black"),
        plot.caption = element_text(hjust=0, size=14,
                                    colour="black"),
...
data <- data %>% 
  mutate(cilindros = as.character(cyl))## se convierte en  carcter para que no lo tome como magnitud 
...
data %>% 
  ...
  scale_x_continuous(breaks = seq(25,350,25)) +
  scale_y_continuous(breaks = seq(10,35,1))+
  scale_color_manual(values=c("#ec7063","#bb8fce","#dc7633"))+
  ...
  theme(legend.position = "bottom",
    panel.background = element_rect(fill="white"),#fondo de la geometria
    plot.background = element_rect(fill = "#abebc6"), #fondo fuera de geometria
    legend.background = element_rect(fill ="#abebc6"),#fondo de la leyenda
    legend.key = element_rect(fill= "#abebc6"),#Fonfo del elemento de la leyenda 
    panel.grid.major = element_line(colour = alpha("#7f8c8d", alpha=0.5)),#color de lineas
    panel.grid.minor = element_blank(),
    plot.title = ggtext::element_markdown(hjust=0.5,#Centra  titulo 
                                          size=18,#Tamaño de letra
...
#Grafico de Radar ----
## Modificar Valores para que esten entre 0 y 1 o en porciento ----
#jugadores$Ritmo <- jugadores$Ritmo/100
#jugadores$Tiro <- jugadores$Tiro/100
#jugadores$Pase <- jugadores$Pase/100
#jugadores$Regate <- jugadores$Regate/100
#jugadores$Defensa <- jugadores$Defensa/100
#jugadores$Fisico <- jugadores$Fisico/100

ggradar(jugadores, grid.min = 0, grid.max = 100)
...
ggradar(jugadores[c(5,8),], grid.min = 0, grid.max = 100,
  ...
  labs(title = "Grafico de radar: Puntaje de caracteristicas de los jugadores",
